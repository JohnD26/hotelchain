...
if(isset($_GET['fetch_rooms']))
{
    ...
    $city = $_GET['city']; // Retrieve the city from the query parameters
    ...
    $city_condition = !empty($city) ? "AND hotels.city LIKE '%" . mysqli_real_escape_string($con, $city) . "%'" : "";
...
/*    $room_res = select("SELECT rooms.*, hotels.hotel_name, hotel_chains.chain_name
                          ...
                          ORDER BY rooms.id DESC", [1, 0], 'ii');*/

    $room_res = select("SELECT rooms.*, hotels.hotel_name, hotel_chains.chain_name 
                          FROM rooms 
                          JOIN hotels ON rooms.hotel_id = hotels.hotel_id 
                          JOIN hotel_chains ON hotels.chain_id = hotel_chains.chain_id 
                          WHERE rooms.status=? AND rooms.removed=? $city_condition 
                          ORDER BY rooms.id DESC", [1, 0], 'ii');
    ...
    while($room_data = mysqli_fetch_assoc($room_res))
    {
        ...
        <div class='card mb-4 border-0 shadow'>
          ...
          <h5 class='mb-3'>{$room_data['hotel_name']}</h5>
