...
if(isset($_POST['add_room'])) {
  // Direct access to 'hotel_id' from the form
  $hotel_id = isset($_POST['hotel_id']) ? (int)$_POST['hotel_id'] : null;

  // Decoding JSON for features and facilities
  ...
  // Filter the entire POST array (assuming filteration is properly handling arrays)
  $frm_data = filteration($_POST);

  // Initialize flag for overall success
  $flag = true;

  // SQL for inserting a new room
  $q1 = "INSERT INTO `rooms` ( `name`, `area`, `price`, `quantity`, `adult`, `children`, `description` , `hotel_id`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
  $values = [
      $frm_data['name'],
      $frm_data['area'],
      $frm_data['price'],
  ...
  // Attempt to insert room details
  if(!insert($q1, $values, 'iisiiiiis')) {
    $flag = false; // Set flag to false if insert fails
  }

  // Retrieve the last inserted room ID
  $room_id = mysqli_insert_id($con);

  // Insert room facilities if room insertion was successful
  if($flag) {
    $q2 = "INSERT INTO `room_facilities`(`room_id`, `facilities_id`) VALUES (?, ?)";
    foreach($facilities as $facility_id) {
      if(!insert($q2, [$room_id, $facility_id], 'ii')) {
        $flag = false; // Set flag to false if any insert fails
        break; // Exit the loop if failure occurs
      }
    }
  }

  // Insert room features if previous insertions were successful
  if($flag) {
    $q3 = "INSERT INTO `room_features`(`room_id`, `features_id`) VALUES (?, ?)";
    foreach($features as $feature_id) {
      if(!insert($q3, [$room_id, $feature_id], 'ii')) {
        $flag = false; // Set flag to false if any insert fails
        break; // Exit the loop if failure occurs
      }
    }
  ...
  // Output based on final outcome
  echo $flag ? "1" : "0"; // Echo '1' for success, '0' for failure
}*/
...
if(isset($_POST['add_room'])) {
  ...
  // Filter the entire POST array
  $frm_data = filteration($_POST);

  // Assuming $frm_data holds all your form data
  foreach ($frm_data as $key => $value) {
    if (in_array($key, ['hotel_id', 'area', 'price', 'quantity', 'adult', 'children'])) {
      $frm_data[$key] = (int) $value;
    }
  }


  // SQL for inserting a new room
  $q1 = "INSERT INTO `rooms` (`name`, `area`, `price`, `quantity`, `adult`, `children`, `description`, `hotel_id`) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
  $values = [
      $frm_data['name'],
      $frm_data['area'],
  ...
  // Insert room details
  $insertResult = insert($q1, $values, 'siiiiiiii');
  if ($insertResult === 0) { // Explicitly check if no rows were affected, indicating failure
  ...
  // Insert facilities
  foreach($facilities as $facility_id) {
    if(!insert("INSERT INTO `room_facilities` (`room_id`, `facilities_id`) VALUES (?, ?)",
        [$room_id, $facility_id], 'ii')) {
      mysqli_rollback($con); // Rollback transaction on failure
      echo "0"; // Indicate failure
      exit;
    }
  ...
  // Insert features
  foreach($features as $feature_id) {
    if(!insert("INSERT INTO `room_features` (`room_id`, `features_id`) VALUES (?, ?)",
        [$room_id, $feature_id], 'ii')) {
      mysqli_rollback($con); // Rollback transaction on failure
      echo "0"; // Indicate failure
      exit;
    }
...
/*if(isset($_POST['add_room'])) {
  ...
  // Decoding JSON for features and facilities and applying filtration if needed
  $features = isset($_POST['features']) ? json_decode($_POST['features'], true) : [];
  $facilities = isset($_POST['facilities']) ? json_decode($_POST['facilities'], true) : [];
  // Filter the entire POST array
  $frm_data = filteration($_POST); // Ensure this function handles arrays properly or adjust accordingly

  $flag = 1; // Assume success initially
  ...
  $values = [
      ...
      $frm_data['adult'],
      $frm_data['children'],
      $frm_data['desc'] // Ensure 'desc' is correctly provided in the form
  ];

  if(!insert($q1, $values, 'iisiiiiis')) {
    $flag = 0; // Set flag to 0 if insert fails
  }

  $room_id = mysqli_insert_id($con); // Retrieve the last inserted id to link with facilities and features
  ...
  if($flag) {
    ...
    foreach($facilities as $facility_id) {
      if(!insert($q2, [$room_id, $facility_id], 'ii')) {
        $flag = 0; // Set flag to 0 if any insert fails
        break; // Exit the loop if failure occurs
      }
    }
  }
...
/*  if(isset($_POST['add_room']))
  {
    ...
    $q1 = "INSERT INTO `rooms` (`name`, `area`, `price`, `quantity`, `adult`, `children`, `description`, `status`, `removed`, `hotel_id`) 
       VALUES (?, ?, ?, ?, ?, ?, ?, 1, 0, ?)";
    $values = [
        $frm_data['name'],
        $frm_data['area'],
        ...
        $frm_data['adult'],
        $frm_data['children'],
        $frm_data['desc'], // Assuming 'desc' is the key for description
        $hotel_id // Now at the end, to match the SQL statement
    ];

    if(!insert($q1, $values, 'iisiiiiii')) { // Updated type string
      $flag = 1;
    }
